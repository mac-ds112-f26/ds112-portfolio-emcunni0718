...
# How many students have filled out the survey?
>44
...
>What is the relationship between a counties median age and the  winner_20 political party?
...
>Why is the Republican support left skewed?
...
#It differs in that we have another axis.
...
>I would say that the relationship is strong. 

-   the direction of the relationship: positive/negative
>The direction of the relationship is posutive.

-   outliers, ie, in what state do counties deviate from the national trend? Any ideas why this might be the case?
>It looks like Texas may have counties that deviate from the nation trend. This may be because Texas is such a big state with so many counities. 
...
ggplot(elections, aes(y = repub_pct_20, x = median_age)) +
  ...
  geom_smooth()
```
>Rent seems to have a negative relationship with the republican percentage in 2020, while age has a more positive relationship.
...
ggplot(elections, aes(y = repub_pct_20, x = historical)) +
  geom_point()
```
>A scatterplot may not be helpful in this because there is so much overalp in the percentages that the data is not statistically significant. It doesn't help answer really any questions since there is so much overlap.
...
>Red states have the highest percentage of repub_pct_20, while blue states have the least. Purple is more of an average between the two but has more overlap with the blue states.
...
```{r}
# harder to read and hard to compare the categories 
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
  geom_density()
```

```{r}
#colors the sections in colors that you want 
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
  geom_density() +
...
```{r}
# changes how translucent the colors are
...
```{r}
# seperate the groups into their own graphs
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
  geom_density() +
  scale_fill_manual(values = c("blue", "purple", "red")) +
...
>I like box plots the most.

-   What is one pro of density plots relative to boxplots?
>You can easily see the most common values/what is the greatest value.

-   What is one con of density plots relative to boxplots?
>It is easier to compare groups and tell if the data is statistically significant in box plots.
...
```{r}
# A stacked bar plot
# The winner is the fill (colors in the bars) and the historical category represents the different bars.
ggplot(elections, aes(x = historical, fill = winner_20)) +
  geom_bar()
```
...
>Pro: It is easier to ccompare the magnitudes/amounts of the different groups.
>Con: It does not provide exact values on the y axis rtaher they are a proportion.

#### Part b

What's your favorite bar plot from part and why?
>I like the stakced bar plot because it is the easiest to read and can help answer many questions regarding the topic.
